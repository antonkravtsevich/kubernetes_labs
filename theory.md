...
2. **Controller Manager** - менеджер контроллеров, отслеживает запуск и работу других контроллеров. Включает:
    ...
    4. **Service Account & Token Controllers** создает аккаунты по умолчанию и выдает токены для доступа новых пользователей
...
Рабочий узел содержит:

1. **kubelet** - управляет контейнерами и подами, отслеживает и обеспечивает их работоспособность. Рабочий агент.
